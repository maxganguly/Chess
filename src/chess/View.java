...
public class View extends JFrame {
	...
	/**
	 * -1 Empty space 0 green Ring, possible move 1 White King 2 White Queen 3 White
	 * Rook 4 White Bishop 5 White Knight 6 White Pawn 7 Black King 8 Black Queen 9
	 * Black Rook 10 Black Bishop 11 Black Knight 12 Black Pawn
	 */
	...
	private Font font;
	private JLabel timeWhite, timeDark;
	...
	public View(String name, int size, boolean timer) {
		...
		this.timeWhite = new JLabel("White: xx:xx");
		JLabel between = new JLabel("    ");
		this.timeDark = new JLabel("Black: xx:xx");
		times.add(this.timeWhite);
		times.add(between);
		times.add(this.timeDark);
		surround.add(BorderLayout.NORTH, times);
	...
	public JLabel[] getTimedisplays() {
		return new JLabel[] {timeWhite,timeDark};
	}
	...
	/**
	 * Draws the Pieces with the index given in toset onto the Board Precondition
	 * toset.length == width && toset[].length == height && given value in toset
	 * must be a vaild index of this.pieces or -1
	 * 
	 * @param toset
	 */
	...
	public Piecetype promote(Team team) {
		...
		Object[] options = (team == Team.WHITE
				? new Object[] { Piecetype.WHITE_QUEEN.symbol, Piecetype.WHITE_ROOK.symbol,
						Piecetype.WHITE_BISHOP.symbol, Piecetype.WHITE_KNIGHT.symbol }
				: new Object[] { Piecetype.DARK_QUEEN.symbol, Piecetype.DARK_ROOK.symbol, Piecetype.DARK_BISHOP.symbol,
						Piecetype.DARK_KNIGHT.symbol });
		int result = JOptionPane.showOptionDialog(this, "Promote to", "Promotion Dialog",
				JOptionPane.INFORMATION_MESSAGE, 0, null, options, options[1]);
	...
	public int gameOver(Team winner) {
		String text = (winner == Team.NONE) ? "Draw \nplay again?"
				: "Player: " + winner.toString() + " won\nplay again?";
		setFont(font);
		int result = JOptionPane.showOptionDialog(this, text, "Game over", JOptionPane.YES_NO_OPTION, 0, null,
